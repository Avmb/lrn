...
log_file = os.path.join(args.save, 'rnd_tune_log.txt')
print('rnd_train load path: {}/model_with_rnd.pt'.format(args.save))
print('log save path: {}'.format(log_file))
print('model save path: {}/model_with_rnd_tune.pt'.format(args.save))
...
try:
    ...
    if args.continue_train:
        optimizer_state = torch.load(os.path.join(args.save, 'rnd_train_optimizer_tune.pt'))
...
#            optimizer = torch.optim.SGD(optimizer_params, lr=args.lr, weight_decay=args.wdecay)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        val_loss2, val_student_loss2 = evaluate(val_data, val_mask, eval_batch_size)
        logging('-' * 89)
        logging('| end of epoch {:3d} | time: {:5.2f}s | valid loss {:5.4f} | '
                'valid ppl {:8.2f} | valid distillation loss {:5.4f}'.format(epoch, (time.time() - epoch_start_time),
                                           val_loss2, math.exp(val_loss2), val_student_loss2))
        logging('-' * 89)

        if val_loss2 < stored_loss:
            save_checkpoint(model, optimizer, args.save, rnd_tune=True)
            logging('Saving!')
...
# Run on test data.
test_loss, test_student_loss = evaluate(test_data, test_mask, test_batch_size)
logging('=' * 89)
logging('| End of training | test loss {:5.2f} | test ppl {:8.2f} | test distillation loss {:5.4f}'.format(
